"""
NavisworksPNT_IFC_Fast — .pnt/IFC export with type-cache and room filter.

Optimizations over NavisworksPNT_IFC:
...
class ElementExtractor:
    ...
    # Whitelist: if non-empty, only these categories are exported.
    # Leave empty to export everything (minus _SKIP_CATEGORIES).
    _ALLOW_CATEGORIES = {
        "Muros", "Walls",
        "Suelos", "Floors",
        "Puertas", "Doors",
    ...
    # Single-fragment-per-instance categories: type cache applied directly,
    # no fragment counting needed.
    _FAST_CATS = {
        "Soportes", "Structural Columns",
        "Vegetación", "Planting",
        "Bordes de losa", "Slab Edges",
    ...
    # Multi-fragment families: full type cache (all frags per instance, per-frag color).
    _INSTNODE_CATS = {
        "Ventanas", "Windows",
        "Muebles de obra", "Casework",
        "Luminarias", "Lighting Fixtures",
    ...
    # Variable-geometry categories: instances of the same type can have different
    # sizes/shapes. Type cache would apply wrong geometry — tessellate inst_node directly.
    _DIRECT_CATS = {
        "Paneles de muro cortina", "Curtain Panels",
        "Montantes de muro cortina", "Curtain Wall Mullions",
        "Muros cortina", "Curtain Walls",
    ...
    # Multi-leaf categories with few leaves: tessellated per-leaf to preserve
    # distinct surface colors (e.g. interior vs exterior wall finish).
    _PERLEAF_CATS = {
        "Muros", "Walls",
        "Techos", "Ceilings",
        "Suelos", "Floors",
    ...
    def __init__(self):
        ...
        self._type_cache = {}   # type_hash -> [(local_v, local_f, color)]
    ...
    def extract(self, model_index: int) -> list:
        ...
        for item in model.RootItem.DescendantsAndSelf:
            if not item.HasGeometry:
                continue
            if self._in_skip_category(item):
                continue
    ...
    @classmethod
    def _in_skip_category(cls, item) -> bool:
        node = item.Parent
        found_category = False
        while node is not None:
            try:
                cls_name = node.ClassDisplayName or ""
                display  = node.DisplayName or ""
                if cls_name in ("Categoría", "Category"):
                    found_category = True
                    if display in cls._SKIP_CATEGORIES:
                        return True
                    if cls._ALLOW_CATEGORIES and display not in cls._ALLOW_CATEGORIES:
                        return True
                    return False
                if not found_category and display in cls._SKIP_CATEGORIES:
                    return True
            except Exception:
                pass
            node = node.Parent
    ...
    @staticmethod
    def _get_category(item) -> str:
        """Return the Revit category display name by walking up to the Category node."""
        node = item.Parent
        while node is not None:
            try:
                cls = node.ClassDisplayName or ""
                if cls in ("Categoría", "Category"):
                    return node.DisplayName or ""
            except Exception:
                pass
            node = node.Parent
        return ""
    ...
    def _flush(self, first_item, items, inst_node):
        ...
        Routing by category eliminates fragment counting (the main perf bottleneck):
          _FAST_CATS   → inst_node + type cache, no frag count (single-frag assumption)
          _PERLEAF_CATS → per-leaf tessellation, no frag count (preserves surface colors)
          default       → count inst_fc once, type cache if single-frag else tessellate inst_node
        """
        if first_item is None:
            return None

        category = self._get_category(first_item)
        sub_meshes = []
        ft = FEET_TO_METERS
        ...
        def _apply_type_cache(inst_node, single_frag_only=False):
            """Type cache: one entry per fragment → (local_v, local_f, color).

            On cache miss (first instance of this type): tessellate each fragment
            in local space (KeepLocal=True) and cache its geometry + color.
            On cache hit: apply each fragment's new world matrix to cached local
            verts, emit one sub_mesh per fragment so each part keeps its color.

            single_frag_only=True: bail immediately if frags != 1 (used for
            _FAST_CATS where a single fragment per instance is guaranteed).
            """
            if inst_node is None:
                return False
            type_node = inst_node.Parent
            type_hash = type_node.GetHashCode() if type_node is not None else None
            if type_hash is None:
                return False
            try:
                inst_path = ComApiBridge.ToInwOaPath(inst_node)
                frags = [CastUtils.CastTo[InwOaFragment3](rf)
                         for rf in inst_path.Fragments()
                         if CastUtils.CastTo[InwOaFragment3](rf) is not None]
                if not frags:
                    return False
                if single_frag_only and len(frags) != 1:
                    return False

                if type_hash in self._type_cache:
                    cached = self._type_cache[type_hash]
                else:
                    # First instance: tessellate each fragment in local space.
                    # Store (frag_idx, local_v, local_f, color) so cache hits
                    # always apply the correct per-fragment matrix even when
                    # some fragments produce no geometry and are skipped.
                    cached = []
                    for i, frag in enumerate(frags):
                        if TriangleMeshCollector.GetMatrix(frag) is None:
                            continue
                        self._collector.Reset()
                        self._collector.KeepLocal = True
                        self._collector.CollectFromFragment(frag)
                        self._collector.KeepLocal = False
                        if self._collector.Faces.Count == 0:
                            continue
                        local_v = [[float(v[0]), float(v[1]), float(v[2])]
                                   for v in self._collector.Vertices]
                        local_f = [[f[0], f[1], f[2]] for f in self._collector.Faces]
                        color = self._item_color(items[i] if i < len(items) else items[0])
                        cached.append((i, local_v, local_f, color))
                    if not cached:
                        return False
                    self._type_cache[type_hash] = cached

                # Emit one sub_mesh per fragment using each entry's stored frag_idx
                # to retrieve the correct per-instance matrix.
                added = False
                for frag_idx, local_v, local_f, color in cached:
                    if frag_idx >= len(frags):
                        continue
                    m = TriangleMeshCollector.GetMatrix(frags[frag_idx])
                    if m is None:
                        continue
                    world_v = [[lv[0]*m[0]+lv[1]*m[4]+lv[2]*m[8] +m[12],
                                 lv[0]*m[1]+lv[1]*m[5]+lv[2]*m[9] +m[13],
                                 lv[0]*m[2]+lv[1]*m[6]+lv[2]*m[10]+m[14]]
                                for lv in local_v]
                    verts = [[v[0]*ft, v[1]*ft, v[2]*ft] for v in world_v]
                    faces = [[f[0]+1, f[1]+1, f[2]+1] for f in local_f]
        ...
        if category in self._PERLEAF_CATS:
            # Per-leaf: preserves distinct surface colors, no fragment counting
            for item in items:
                verts, faces = _tessellate(item)
                if verts:
                    sub_meshes.append((verts, faces, self._item_color(item)))

        elif category in self._FAST_CATS:
            # Known single-fragment: type cache, no frag counting
            if not _apply_type_cache(inst_node, single_frag_only=True):
                target = inst_node if inst_node is not None else items[0]
                verts, faces = _tessellate(target)
                if verts:
                    sub_meshes.append((verts, faces, self._item_color(items[0])))

        elif category in self._INSTNODE_CATS:
            # Multi-fragment families: full type cache (all frags per instance)
            if not _apply_type_cache(inst_node, single_frag_only=False):
                target = inst_node if inst_node is not None else items[0]
                verts, faces = _tessellate(target)
                if verts:
                    sub_meshes.append((verts, faces, self._item_color(items[0])))

        elif category in self._DIRECT_CATS:
            # Variable-geometry: tessellate inst_node directly in world space.
            # No cache — geometry differs per instance (size, configuration).
            target = inst_node if inst_node is not None else items[0]
            verts, faces = _tessellate(target)
            if verts:
                sub_meshes.append((verts, faces, self._item_color(items[0])))
            ...
            # Unknown category: type cache (handles any frag count)
            if not _apply_type_cache(inst_node, single_frag_only=False):
                target = inst_node if inst_node is not None else items[0]
                verts, faces = _tessellate(target)
                if verts:
