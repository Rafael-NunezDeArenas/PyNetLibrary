...
class PyNETSharedParamCreator:
    ...
    PARAMS = [
        ...
        ("PyNET_4D", "PyNET_4D_CodigoRecurso",        "TEXT",    False, "4D"),
    ...
    def create_shared_param_file():
        ...
        sp_path = str(Path(desktop) / "PyNET_SharedParameters.txt")
    ...
    def Run():
        ...
        print(f"Shared parameters file: {sp_path}")

        definitions = PyNETSharedParamCreator.create_definitions(def_file, spec_map)
        print(f"Shared parameters created: {len(definitions)}")

        t = Transaction(doc, "PyNET - Create project parameters")
        t.Start()
        try:
            ok = PyNETSharedParamCreator.bind_to_categories(definitions)
            t.Commit()
            print(f"Project parameters bound: {ok}/{len(definitions)}")
